...
export const ReferralTabListItem = ({ isSelect = false, id, newUsers, numUsers, noCursor = false, onClick = () => {} }: IProps) => {
  return (
    <li className={`bg-white rounded-tl-2xl relative mb-4 h-24 list-none last:mb-0 ${noCursor ? '' : 'shadow'}`}>
      {isSelect && <div style={{ top: '35px' }} className='absolute left-2 rounded-full bg-primary-500 w-2 h-6' />}
      <button
      ...
      >
        <div className='flex items-center justify-start'>
          <span className='text-black text-xl font-bold'>Level {id}</span>
          <div className='block my-1 text-[#7194AF] text-lg font-semibold'><span className="py-1 px-4 mx-2 font-bold rounded-lg text-primary-500 border border-primary-500">{numUsers}</span> Users</div>
