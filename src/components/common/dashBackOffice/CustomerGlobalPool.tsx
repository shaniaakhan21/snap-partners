...
export default function CustomerGlobalPool ({ data }: { data: any}) {
  const totalCV = data?.totalCV || 0
  const shares = Math.floor(totalCV / 200)
  const needed = 200 - Math.floor(totalCV - (shares * 200))
  const percents = ((200 - needed) * 100) / 200
  const amounts = {
    shares: Math.floor(totalCV / 200),
    needed: needed,
      ...
      <div className="w-full max-w-full p-4 space-y-2 bg-white rounded-xl mt-4">
        ...
        <div className="flex flex-row w-full items-center">
          ...
          <div className="flex flex-col items-center w-1/2 items-center ">
            ...
            <div className='p-1'>
              <h1 className="text-base text-center pt-2">Total CV for the Month</h1>
              <h1 className="text-base text-center pt-2"><strong>{totalCV}cv</strong></h1>
            </div>
          </div>
        </div>
